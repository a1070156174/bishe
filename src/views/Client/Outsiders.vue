  ...
  <div class="IndexClient">
     <div class="search-area">
       <el-form :inline="true" :model="query" class="demo-form-inline">
        <el-form-item label="外卖人员身份证号">
          <el-input v-model="query.cardId" placeholder="ID"></el-input>
        </el-form-item>
     ...
     <div class="table-area">
        ...
        <el-table-column  prop="time" :formatter="formatDate" label="日期"    width="180"></el-table-column>
        <el-table-column  prop="name" label="姓名" width="180"></el-table-column>
        <el-table-column  prop="entyTime" label="进入时间" width="180"></el-table-column>
        <el-table-column  prop="door" label="门岗" width="180"></el-table-column>
        <el-table-column prop="cardId" label="身份证号" ></el-table-column>
        <el-table-column  prop="outTime" label="出去时间" width="180"></el-table-column>
    ...
    <el-pagination background class="lc-pagination"
     ...
     <Outsiders-info ref="outsidersInfo"></Outsiders-info>
  </div>
</template>

<script>
import OutsidersInfo from '../Form/OutsidersInfo'
import{Oudsidersgetform,Oudsidersselect,Oudsidersdel,getdepartment}from '@/http/api.js'
export default {
  name: 'Home',
  components:{
    OutsidersInfo
  ...
  methods:{
    ...
    doQuery(){
      console.log('查询事件')
      Oudsidersselect(this.query).then(res=>{
        console.log('查询成功')
        this.$message({ message: '恭喜你!查询成功', type: 'success'})
        this.tableData=res.data
        // this.$refs.takeOutInfo.dname=this.department
      })
    },
    //新增
    doNew(){
       this.$refs.outsidersInfo.dialogVisible = true
       this.$refs.outsidersInfo.switch = 'add'
       this.$refs.outsidersInfo.dname=this.department
...
        const tHeader = ["日期", "姓名","进入时间","门岗","身份证号","出去时间","备注"];

        //与表头相对应的数据里边的字段

        const filterVal = ["time","name","entyTime","door","cardId","outTime", "remarks"];
...
        excel.export_json_to_excel(tHeader,data,'外来人员表');
...
      });
    ...
    doCheck(row){
       this.$refs.outsidersInfo.dialogVisible = true
       this.$refs.outsidersInfo.switch = 'update'
       this.$refs.outsidersInfo.takeOut = row
      this.$refs.outsidersInfo.dname=this.department
    },
    doDelete(row){
      console.log('删除事件')
      Oudsidersdel({
        ...
        console.log('删除成功')
         this.$message({  message: '恭喜你！删除'+row.wid+'成功！', type: 'success'  })
         this.doQuery()
      })
    },
